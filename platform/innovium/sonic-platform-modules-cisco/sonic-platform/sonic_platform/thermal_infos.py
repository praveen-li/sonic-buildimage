...
class FanInfo(ThermalPolicyInfoBase):
    ...
    def __init__(self):
        ...
        self._psu_fans = set()
        self._psu_fan_status_changed = False
    ...
    def collect(self, chassis):
        ...
        self._psu_status_changed = False
        for psu_num in chassis.get_all_psus():
            for psu_fan_num in psu_num.get_all_fans():
                psu_fan_presence = psu_fan_num.get_presence()
                if psu_fan_presence and psu_fan_num not in self._psu_fans:
                    self._psu_fans.add(psu_fan_num)
                    self._psu_fan_status_changed = True
                else:
                    if psu_fan_num in self._psu_fans:
                        self._psu_fans.remove(psu_fan_num)
                        self._psu_fan_status_changed = True
    ...
    def get_updated_psu_fans(self):
        """
        Retrieves psu fans
        :return: A set of psu fans
        """
        return self._psu_fans

    def is_psu_fan_status_changed(self):
        """
        Retrieves if the status of psu fan information changed
        :return: True if status changed else False
        """
        return self._psu_fan_status_changed
