    ...
    def is_match(self, thermal_info_dict):
        ...
        if fan_info_obj and sensor_info_obj and chassis_info_obj:
            ...
            if n_blue_fans and n_red_fans:
                ...
                ambient_high_threshold = platform_data.get_param(PlatformGlobalData.KEY_BLUE_FANS_INLET_TEMP_HIGH_THRESHOLD,
                        Chassis.DEFAULT_AMBIENT_TEMP_THRESHOLD)
            else:
                ambient_high_threshold = platform_data.get_param(PlatformGlobalData.KEY_RED_FANS_INLET_TEMP_HIGH_THRESHOLD,
                        Chassis.DEFAULT_AMBIENT_TEMP_THRESHOLD)
            
            ambient_temperature = sensor_info_obj.get_ambient_temperature()
            if (ambient_temperature >= ambient_high_threshold):
                return True
            else:
                return False
        else:
...
@thermal_json_object('sensor.ambient.warm')
class AnyAmbientSensorWarmCondition(SensorCondition):
    def is_match(self, thermal_info_dict):
        sensor_info_obj = self.get_sensor_info(thermal_info_dict)
        chassis_info_obj = self.get_chassis_info(thermal_info_dict)
        fan_info_obj = self.get_fan_info(thermal_info_dict)
        ...
        if fan_info_obj and sensor_info_obj and chassis_info_obj:
            ...
            if n_blue_fans and n_red_fans:
                ...
                ambient_low_threshold = platform_data.get_param(PlatformGlobalData.KEY_BLUE_FANS_INLET_TEMP_LOW_THRESHOLD,
                        Chassis.DEFAULT_AMBIENT_TEMP_THRESHOLD)
                ambient_high_threshold = platform_data.get_param(PlatformGlobalData.KEY_BLUE_FANS_INLET_TEMP_HIGH_THRESHOLD,
                        Chassis.DEFAULT_AMBIENT_TEMP_THRESHOLD)
            else:
            ...
            ambient_temperature = sensor_info_obj.get_ambient_temperature()
            if (ambient_temperature >= ambient_low_threshold and ambient_temperature < ambient_high_threshold):
                return True
            else:
                return False
        else:
    ...
    def is_match(self, thermal_info_dict):
        ...
        if fan_info_obj and sensor_info_obj and chassis_info_obj:
            ...
            if n_blue_fans and n_red_fans:
                ...
                ambient_low_threshold = platform_data.get_param(PlatformGlobalData.KEY_RED_FANS_INLET_TEMP_LOW_THRESHOLD,
                        Chassis.DEFAULT_AMBIENT_TEMP_THRESHOLD)

            ambient_temperature = sensor_info_obj.get_ambient_temperature()
            if (ambient_temperature < ambient_low_threshold):
                return True
            else:
                return False
        else:
